...
public class FoldingCell extends RelativeLayout {
    ...
    public FoldingCell(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray styledAttrs = context.obtainStyledAttributes(attrs, com.ramotion.foldingcell.R.styleable.FoldingCell);
        if (styledAttrs!=null){
            final int count = styledAttrs.getIndexCount();
            for (int i = 0; i < count; ++i) {
                int attr = styledAttrs.getIndex(i);
                if (attr == com.ramotion.foldingcell.R.styleable.FoldingCell_animationDuration) {
                    this.mAnimationDuration = styledAttrs.getInt(com.ramotion.foldingcell.R.styleable.FoldingCell_animationDuration, DEF_ANIMATION_DURATION);
                } else if (attr == com.ramotion.foldingcell.R.styleable.FoldingCell_backSideColor) {
                    this.mBackSideColor = styledAttrs.getColor(com.ramotion.foldingcell.R.styleable.FoldingCell_backSideColor, DEF_BACK_SIDE_COLOR);
                } else if (attr == com.ramotion.foldingcell.R.styleable.FoldingCell_additionalFlipsCount) {
                    this.mAdditionalFlipsCount = styledAttrs.getInt(com.ramotion.foldingcell.R.styleable.FoldingCell_additionalFlipsCount, DEF_ADDITIONAL_FLIPS);
                } else if (attr == com.ramotion.foldingcell.R.styleable.FoldingCell_cameraHeight) {
                    this.mCameraHeight = styledAttrs.getInt(com.ramotion.foldingcell.R.styleable.FoldingCell_cameraHeight, DEF_CAMERA_HEIGHT);
                }
            }
            styledAttrs.recycle();
        }
